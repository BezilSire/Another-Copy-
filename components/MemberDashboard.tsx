...
export const MemberDashboard: React.FC<MemberDashboardProps> = ({ user, onUpdateUser, unreadCount, onLogout, onViewProfile, forcedView, clearForcedView }) => {
  ...
  const handlePostCreated = (ccapAwarded: number) => {
    addToast(ccapAwarded > 0 ? `Posted! Earned ${ccapAwarded} points.` : 'Posted successfully.', 'success');
  ...
  const handleUpgradeComplete = async (mnemonic: string, pin: string) => {
    ...
    addToast("Identity secured successfully.", "success");
  };
  ...
  const renderMainContent = () => {
    ...
    if (isAnonymous && !['meeting', 'knowledge', 'home'].includes(view)) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center px-6 font-sans">
                <div className="p-5 bg-slate-800 rounded-full mb-6 text-slate-500"><LockIcon className="h-10 w-10" /></div>
                <h3 className="text-xl font-bold text-white mb-2">Member Feature</h3>
                <p className="text-sm text-slate-400 mb-8 max-w-xs">Please create a full account to access community features.</p>
                <button onClick={() => window.location.reload()} className="px-8 py-3 bg-brand-gold text-slate-900 font-bold rounded-xl shadow-lg hover:bg-brand-goldlight transition-colors">Join Now</button>
            </div>
        );
    }
