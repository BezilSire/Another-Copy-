...
const NavItem: React.FC<{
  ...
  >
    <div className={`flex-shrink-0 p-3 rounded-xl ${highlight ? 'bg-brand-gold/10 text-brand-gold' : 'bg-slate-800 text-white'}`}>{icon}</div>
    <span className="flex-1 ml-5 text-left font-black uppercase tracking-[0.2em] text-[10px] text-white">{label}</span>
    {badgeCount !== undefined && badgeCount > 0 && (
      <span className="bg-red-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full">{badgeCount}</span>
    )}
    <ChevronRightIcon className="h-4 w-4 text-white/40 ml-2" />
  </button>
);
    ...
    <div className="animate-fade-in space-y-10 max-w-2xl mx-auto pb-20">
      <div className="flex items-center space-x-6 p-8 bg-slate-950/60 rounded-[3rem] border border-white/5 shadow-2xl relative overflow-hidden">
        ...
        <div>
          ...
          <p className="label-caps !text-[9px] !text-white mt-2 font-black !tracking-[0.4em]">Protocol Node Operational</p>
        </div>
      </div>

      <div className="space-y-3">
        <h3 className="label-caps !text-[9px] !text-white pl-4 mb-4">Node Operations</h3>
        ...
        <NavItem icon={<MessageSquareIcon className="h-4 w-4" />} label="Comms Hub" onClick={() => onNavigate('chats')} />
        <NavItem icon={<LockIcon className="h-4 w-4" />} label="Identity Vault" onClick={() => onNavigate('security')} />
      </div>

      <div className="space-y-3">
        <h3 className="label-caps !text-[9px] !text-white pl-4 mb-4">Network State</h3>
        <NavItem icon={<DatabaseIcon className="h-4 w-4" />} label="The Registry (Assets)" onClick={() => onNavigate('registry')} />
        <NavItem icon={<ShieldCheckIcon className="h-4 w-4" />} label="Sovereign State" onClick={() => onNavigate('state')} />
      ...
      <div className="space-y-3">
        <h3 className="label-caps !text-[9px] !text-white pl-4 mb-4">Ecosystem Modules</h3>
