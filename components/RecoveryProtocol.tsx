...
export const RecoveryProtocol: React.FC<RecoveryProtocolProps> = ({ onComplete, onBack }) => {
    ...
    const handleReset = async () => {
        ...
        // Execute completion and force immediate return to dashboard via props
        await onComplete(mnemonic, pin, vaultData);
        setIsVerifying(false);
    };
                        ...
                        <div className="pt-8 border-t border-white/5 space-y-6 text-center">
                            <button 
                                ...
                                onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    if(window.confirm("ACCOUNT_RECOVERY: Overwrite local vault using cloud credentials?")) {
