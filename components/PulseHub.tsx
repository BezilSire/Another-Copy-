...
export const PulseHub: React.FC<PulseHubProps> = ({ user }) => {
    ...
    const handleSwapInitiate = async () => {
        ...
        if (swapType === 'BUY') setShowEcocashBridge(true);
        else setShowSellConfirmation(true);
    };
    ...
    const handleFinalSell = async () => {
        ...
        try {
            ...
            addToast("Liquidation Protocol Initiated.", "success");
            setShowSellConfirmation(false);
            setSwapAmount('');
        } catch (e: any) {
            addToast(e.message || 'Liquidation Error.', 'error');
        } finally {
            setIsSwapping(false);
        }
    };
    ...
    const handleEcocashSubmit = async () => {
        ...
        try {
            ...
            addToast("Handshake Pending Verification.", "success");
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-24 px-4 font-sans">
            ...
            {/* TERMINAL HEADER: Index & Pool State */}
            ...
            {view === 'swap' ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    {/* Swap Terminal */}
                    <div className="lg:col-span-8 module-frame bg-slate-950/60 p-8 sm:p-12 rounded-[3.5rem] border-white/5 shadow-premium space-y-10">
                        <div className="flex justify-between items-center border-b border-white/5 pb-8">
                            <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Liquidity Dispatch</h2>
                            <div className="flex bg-black/60 p-1.5 rounded-xl border border-white/5">
                                <button onClick={() => setSwapType('BUY')} className={`px-6 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${swapType === 'BUY' ? 'bg-emerald-600 text-white shadow-lg' : 'text-gray-500'}`}>Buy</button>
                                <button onClick={() => setSwapType('SELL')} className={`px-6 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${swapType === 'SELL' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-500'}`}>Sell</button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-8">
                            <div className="bg-black/40 p-8 rounded-[2.5rem] border border-white/5 focus-within:border-brand-gold/30 transition-all shadow-inner">
                                <div className="flex justify-between items-center mb-4">
                                    <label className="label-caps !text-[10px]">Asset Volume</label>
                                    <span className="text-[10px] font-bold text-gray-600 font-mono">{(user.ubtBalance || 0).toFixed(2)} AVAILABLE</span>
                                </div>
                            ...
                            <div className="bg-black/40 p-8 rounded-[2.5rem] border border-white/5 shadow-inner">
                                <label className="label-caps !text-[10px] mb-4 block">Settlement Estimate</label>
                                <div className="flex justify-between items-center">
                                    <p className="text-6xl font-black font-mono text-white tracking-tighter leading-none">${estTotal.toFixed(2)}</p>
                                    <span className="text-xl font-black text-gray-700 font-mono tracking-widest uppercase">USD</span>
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            {isSwapping ? <LoaderIcon className="h-5 w-5 animate-spin" /> : <>Initiate Protocol Signature <ShieldCheckIcon className="h-5 w-5"/></>}
                        </button>
                    </div>
                                    ...
                                    <div key={req.id} className="p-5 bg-black/40 rounded-2xl border border-white/5 space-y-4 group hover:border-brand-gold/20 transition-all">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest mt-2">Target: ${req.amountUsd.toFixed(2)}</p>
                                            </div>
                                        ...
                                        {req.status === 'DISPATCHED' && (
                                            <div className="pt-2 animate-fade-in space-y-3">
                                                <p className="text-[9px] text-emerald-400 uppercase font-black tracking-widest leading-loose">Internal Payout Received? Verify SMS Reference below.</p>
                                                <div className="p-3 bg-black rounded-xl border border-white/5 data-mono text-xs text-white break-all">{req.ecocashRef}</div>
                                                <button onClick={() => setRequestToConfirmReceipt(req)} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Verify & Release</button>
                                            </div>
                                        )}
                                        {req.status === 'PENDING' && (
                                             <button onClick={() => api.cancelSellRequest(user, req.id)} className="w-full py-2 text-[8px] text-gray-700 hover:text-red-500 font-black uppercase tracking-[0.3em] transition-colors">Abort Session</button>
                                        )}
                                    </div>
                                ))}
                ...
                <div className="animate-fade-in space-y-6">
                    <div className="flex justify-between items-center px-4">
                        <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Trading Floor</h2>
                        <button className="flex items-center gap-3 px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">
                             <PlusIcon className="h-4 w-4 text-brand-gold" /> Post Offer
                        </button>
                    </div>
                        ...
                        {offers.map(offer => (
                            <div key={offer.id} className="module-frame bg-slate-900/60 p-6 rounded-[2.5rem] border-white/5 hover:border-brand-gold/20 transition-all flex flex-col gap-6 relative group shadow-xl">
                                ...
                                <div className="pt-6 border-t border-white/5 flex justify-between items-center">
                                    <div>
                                        <p className="text-[8px] font-black text-gray-700 uppercase tracking-[0.3em] mb-1">Settlement Method</p>
                    ...
                    {offers.length === 0 && (
                        <div className="py-40 text-center glass-card rounded-[4rem] border-white/5 opacity-30">
                            <UsersIcon className="h-16 w-16 mx-auto mb-6 text-gray-800" />
                            <p className="label-caps !text-[12px] !tracking-[0.6em]">No Active Floor Orders</p>
                        </div>
                    )}
                </div>
            )}
            ...
            <ConfirmationDialog 
                ...
                message={`Verify disposal of ${swapAmount} UBT. Assets will be escrowed by the Treasury until settlement confirmation.`}
                confirmButtonText="Authorize Liquidation"
            />
            ...
            <ConfirmationDialog 
                ...
                onConfirm={async () => {
                    ...
                    try {
                        ...
                        addToast("Ledger Settled. UBT Released.", "success");
                ...
                message={`Confirm receipt of $${requestToConfirmReceipt?.amountUsd.toFixed(2)} via Ecocash. This signs the final block and releases your UBT stake.`}
                confirmButtonText="Settle Handshake"
            />

            {showEcocashBridge && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/95 backdrop-blur-2xl" onClick={() => setShowEcocashBridge(false)}></div>
                    <div className="module-frame bg-slate-950 w-full max-w-lg p-10 rounded-[3.5rem] border-brand-gold/30 z-10 relative space-y-10 animate-fade-in shadow-[0_0_100px_-20px_rgba(212,175,55,0.1)]">
                        <div className="corner-tl"></div><div className="corner-br"></div>
                        <div className="flex justify-between items-center">
                            <div>
                                <h3 className="text-2xl font-black text-white uppercase tracking-tighter gold-text leading-tight">Handshake Bridge</h3>
                                <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">Ecocash Gateway Protocol</p>
                            </div>
                            <button onClick={() => setShowEcocashBridge(false)} className="p-2 text-gray-600 hover:text-white"><XCircleIcon className="h-8 w-8" /></button>
                        </div>
                        <div className="space-y-8">
                            <div className="bg-black/60 p-8 rounded-[2rem] border border-brand-gold/20 shadow-inner text-center">
                                 <p className="text-[9px] font-black text-gray-500 uppercase tracking-[0.4em] mb-4">Command Sequence</p>
                                 <p className="text-3xl font-black text-brand-gold font-mono tracking-tighter break-all select-all">{ussdCommand}</p>
                            </div>
                            <div className="space-y-4">
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1">Sync Reference Proof (SMS)</label>
                                <input 
                                    type="text" 
                                    value={ecocashRef} 
                                    onChange={e => setEcocashRef(e.target.value.toUpperCase())}
                                    placeholder="PP230524.XXXX.XXXX"
                                    className="w-full bg-black border border-white/10 p-5 rounded-2xl text-white font-mono text-xl focus:outline-none focus:ring-1 focus:ring-brand-gold/30 uppercase shadow-inner"
                                />
                            </div>
                        </div>
                        <button 
                            ...
                            className="w-full py-6 bg-brand-gold text-slate-950 font-black rounded-3xl uppercase tracking-[0.3em] text-xs shadow-glow-gold active:scale-95 transition-all flex justify-center items-center gap-2"
                        >
                            {isSwapping ? <LoaderIcon className="h-5 w-5 animate-spin" /> : "Verify & Synchronize Node"}
                        </button>
                    </div>
                </div>
            )}
