...
const PulseCandleChart: React.FC<{ ledger: UbtTransaction[], currentPrice: number }> = ({ ledger, currentPrice }) => {
    const candles = useMemo(() => {
        ...
        // Filter market affecting events
        ...
        for (let i = 0; i < candleCount; i++) {
            ...
            const impact = chunk.reduce((acc, tx) => {
                const volFactor = tx.amount / 400000; 
                return tx.senderId === 'FLOAT' ? acc - volFactor : acc + volFactor;
            }, 0) || (Math.random() - 0.5) * 0.00002; 

            const open = close + impact;
            const high = Math.max(open, close) + Math.abs(impact * 0.3);
            const low = Math.min(open, close) - Math.abs(impact * 0.3);
    ...
    return (
        <div className="w-full h-44 relative overflow-hidden mt-8 bg-slate-950/60 rounded-3xl border border-white/5 shadow-inner group">
            <div className="absolute inset-0 z-0 opacity-[0.04] pointer-events-none" 
                 style={{ backgroundImage: 'linear-gradient(rgba(212,175,55,1) 1px, transparent 1px), linear-gradient(90deg, rgba(212,175,55,1) 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
            </div>

            <svg viewBox="0 0 400 100" className="w-full h-full p-4 relative z-10 overflow-visible">
                {candles.map((c, i) => {
                    const x = (i / (candles.length - 1)) * 370 + 15;
                    ...
                    const bodyH = Math.max(Math.abs(yOpen - yClose), 1.2);

                    return (
                        <g key={i} className="animate-fade-in" style={{ animationDelay: `${i * 30}ms` }}>
                            {/* Glow Trace */}
                            <rect x={x - 4} y={bodyY} width="8" height={bodyH} fill={color} opacity="0.1" className="blur-sm" />
                            {/* Wick */}
                            <line x1={x} y1={yHigh} x2={x} y2={yLow} stroke={color} strokeWidth="0.8" opacity="0.5" />
                            {/* Body */}
                            <rect 
                                x={x - 4} 
                                y={bodyY} 
                                ...
                                fill={color} 
                                rx="0.5" 
                                className={`transition-all duration-700 ${i === candles.length - 1 ? 'animate-pulse' : ''}`} 
                            />
                        </g>
                    );
                })}
            ...
            <div className="absolute top-4 left-6 flex items-center gap-3">
                <div className="flex flex-col">
                    <span className="text-[7px] font-black text-gray-600 uppercase tracking-[0.4em]">Spectrum_Trace_Live</span>
                    <div className="w-24 h-0.5 bg-brand-gold/20 rounded-full overflow-hidden mt-1">
                        <div className="h-full bg-brand-gold w-1/3 animate-[translateX_2s_infinite_linear]" style={{animation: 'scan 2s infinite linear'}}></div>
                    </div>
                </div>
            </div>
            <style>{`
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="bg-black/40 p-10 rounded-[3rem] border border-white/5 shadow-inner">
                                        ...
                                        <div className="flex justify-between items-center">
                                            <p className="text-7xl font-black font-mono text-white tracking-tighter leading-none">${estTotal.toFixed(6)}</p>
                                            <span className="text-2xl font-black text-gray-700 font-mono tracking-widest uppercase">USD</span>
                                        </div>
                                    </div>
                                </div>

                                <button onClick={() => {}} disabled={isSwapping || !swapAmount} className={`w-full py-8 font-black rounded-3xl transition-all active:scale-[0.98] shadow-glow-gold uppercase tracking-[0.5em] text-[11px] flex justify-center items-center gap-4 ${swapType === 'SELL' ? 'bg-red-600 text-white' : 'bg-brand-gold text-slate-950'}`}>
                                    {isSwapping ? <LoaderIcon className="h-6 w-6 animate-spin" /> : <>Authorize Handshake <ShieldCheckIcon className="h-6 w-6"/></>}
                                </button>
                            </div>
                        ) : (
                            <div className="py-20 flex flex-col items-center"><LoaderIcon className="h-10 w-10 animate-spin text-brand-gold"/></div>
                        )}
                    </div>
                                    ...
                                    <div key={req.id} className="p-6 bg-black/40 rounded-3xl border border-white/5">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <p className="text-[9px] font-bold text-gray-600 uppercase mt-2 tracking-widest">${req.amountUsd.toFixed(6)} USD</p>
                                            </div>
                ...
                <div className="animate-fade-in grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                     {offers.map(offer => (
                        <div key={offer.id} className="module-frame glass-module p-8 rounded-[3rem] border-white/5 hover:border-brand-gold/30 transition-all flex flex-col gap-8 shadow-2xl relative group">
                            ...
                            <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Rate: ${offer.pricePerUnit.toFixed(6)}</p>
