...
import { VentureDetailsAdmin } from './VentureDetailsAdmin';

type VentureFilter = 'fundraising' | 'operational' | 'all';
...
export const VenturesAdminPage: React.FC<VenturesAdminPageProps> = ({ user, ventures }) => {
    const [filter, setFilter] = useState<VentureFilter>('fundraising');
    ...
    if (selectedVenture) {
        return <VentureDetailsAdmin venture={selectedVenture} onBack={() => setSelectedVenture(null)} />;
    }
    ...
    return (
        <div className="bg-slate-800 p-6 rounded-lg shadow-lg">
            ...
            <div className="flex space-x-1 bg-slate-700 p-1 rounded-lg mb-4 max-w-md">
                 <button onClick={() => setFilter('fundraising')} className={`w-full px-4 py-2 text-sm font-medium rounded-md ${filter === 'fundraising' ? 'bg-slate-900 text-white' : 'text-gray-300 hover:bg-slate-600'}`}>Fundraising</button>
                        ...
                        {filteredVentures.map(v => (
                            <tr key={v.id}>
                                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-white sm:pl-0">
                                    <button onClick={() => setSelectedVenture(v)} className="hover:underline text-left">{v.name}</button>
                                </td>
