...
export const UserCard: React.FC<UserCardProps> = ({ user, currentUser, onClick, isOnline }) => {
  ...
  const handleFollowToggle = async (e: React.MouseEvent) => {
    ...
    try {
        ...
        addToast('Action failed. Please check your connection.', 'error');
    } finally {
        setIsLoading(false);
    }
  };
        ...
        <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3 min-w-0">
                ...
                <div className="relative flex-shrink-0">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-slate-700 flex items-center justify-center overflow-hidden ring-1 ring-slate-600 group-hover:ring-green-500/50 transition-all">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                        ...
                        <span className="text-[10px] text-gray-400 bg-slate-900/50 px-2 py-0.5 rounded-full truncate max-w-[100px] border border-slate-700">{user.circle}</span>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div className="bg-slate-900/30 px-4 py-3 border-t border-slate-700/50 flex items-center justify-between gap-2 mt-auto">
          ...
          {!isOwnProfile && (
            <button
                ...
                disabled={isLoading}
                className={`
                    flex-shrink-0 flex items-center justify-center h-8 px-3 rounded-full text-xs font-bold transition-all
                    ${isFollowing 
