...
const knowledgeContent = [
  ...
  },
  {
    ...
    text: "We are the generation that will end economic dependence and institutional deceit. We are not the elite, yet we are the essential. We feed the system  and now, we have built one that feeds us.\n\nThe Global Commons Network is not a company. It is not a foundation. It is a living social organism  a system that grows stronger as its people grow.\n\nIn time, $UBT will become the most valuable asset class in Zimbabwe  not because of speculation, but because it represents the very heartbeat of human necessity. When the world realises that the power of wealth lies not in scarcity but in shared trust, the Global Commons Network will stand as proof.\n\nThis is not charity. This is not politics. This is the rebirth of economic dignity."
  }
];
            ...
            ([entry]) => {
                if (entry.isIntersecting && !currentUser.hasReadKnowledgeBase) {
                    api.awardKnowledgePoints(currentUser.id).then(wasAwarded => {
                        if (wasAwarded) {
                            addToast('Thank you for reviewing the knowledge base!', 'success');
                            // Optimistically update the user object for the UI
                            onUpdateUser({ hasReadKnowledgeBase: true });
                        }
                    });
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in space-y-8">
            <div className="text-center p-6 bg-slate-800 rounded-lg">
                ...
                <h1 className="text-3xl font-bold text-white">GLOBAL COMMONS NETWORK</h1>
