...
type NavView = 'profile' | 'notifications' | 'sustenance' | 'myinvestments' | 'knowledge' | 'launchpad' | 'earn' | 'ledger' | 'security';
  ...
  return (
    <div className="animate-fade-in space-y-10 max-w-2xl mx-auto pb-20">
      <div className="flex items-center space-x-6 p-8 bg-slate-950/60 rounded-[3rem] border border-white/5 shadow-2xl relative overflow-hidden">
        ...
        <div>
          <h1 className="text-3xl font-black text-white uppercase tracking-tighter gold-text">{user.name}</h1>
          <p className="label-caps !text-[9px] !text-gray-500 mt-1">{user.role} &bull; Node operational</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <NavItem icon={<DatabaseIcon className="h-4 w-4" />} label="Contribution Engine" onClick={() => onNavigate('earn')} highlight />
      ...
      <div className="space-y-3">
        ...
        <NavItem icon={<TrendingUpIcon className="h-4 w-4" />} label="Venture Equity" onClick={() => onNavigate('myinvestments')} />
        <NavItem icon={<HeartIcon className="h-4 w-4" />} label="Sustenance Dividend" onClick={() => onNavigate('sustenance')} />
        <NavItem icon={<SparkleIcon className="h-4 w-4" />} label="Project Launchpad" onClick={() => onNavigate('launchpad')} />
