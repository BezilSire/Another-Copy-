...
export const PinVaultLogin: React.FC<PinVaultLoginProps> = ({ onUnlock, onReset }) => {
    ...
    return (
        <div className="module-frame glass-module p-10 sm:p-14 rounded-[4rem] border-white/20 shadow-premium animate-fade-in max-w-md w-full flex flex-col items-center bg-slate-950 relative z-50">
                    ...
                    {pin.map((digit, i) => (
                        <div key={i} className="relative flex-1">
                            <input
                                ...
                                className={`w-full aspect-[4/5] bg-black border-[3px] rounded-2xl text-center text-5xl font-black transition-all outline-none
            ...
            <div className="mt-14 w-full pt-10 border-t border-white/5 space-y-8 pointer-events-auto">
                <button 
                    type="button"
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onReset(); }} 
                    className="w-full flex items-center justify-center gap-3 py-4 text-[11px] font-black text-brand-gold/60 hover:text-brand-gold uppercase tracking-[0.4em] transition-all group cursor-pointer"
                >
                ...
                <div className="p-8 bg-red-950/20 border border-red-900/30 rounded-[3rem] space-y-4 text-center shadow-2xl">
                    ...
                    <button 
                        type="button"
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); onReset(); }}
                        className="flex items-center justify-center gap-4 mx-auto py-4 px-10 bg-red-600 hover:bg-red-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-premium active:scale-95 transition-all cursor-pointer"
                    >
