...
export const PulseHub: React.FC<PulseHubProps> = ({ user }) => {
    ...
    const ubtOutput = useMemo(() => {
        ...
        return Number((usd / ubtPrice).toFixed(4));
    }, [usdInput, ubtPrice]);
    ...
    const handleFinalize = async () => {
        ...
        setIsProcessing(true);
        try {
            await api.updatePendingPurchaseReference(lastPurchaseId, ecocashRef.trim().toUpperCase());
            addToast("Reference Linked. Awaiting Oracle signature.", "success");
            setBuyState('input');
            setUsdInput('');
            setEcocashRef('');
        } catch (e) {
    ...
    return (
        <div className="max-w-md mx-auto space-y-6 animate-fade-in pb-24 px-4 font-sans">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-slate-900 border border-white/5 p-5 rounded-[1.5rem] shadow-xl">
                    <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-1.5">Pool Liquidity</p>
