...
const App: React.FC = () => {
  ...
  const renderMainContent = () => {
    ...
    // Joint Hub for all Anonymous nodes
    if (firebaseUser?.isAnonymous) {
        if (activeMeetingId && currentUser) return <VideoMeeting user={currentUser} meetingId={activeMeetingId} isHost={false} onEnd={() => { setActiveMeetingId(null); window.history.pushState({}, '', window.location.pathname); }} />;
